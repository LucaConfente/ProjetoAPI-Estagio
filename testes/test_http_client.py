...
def test_erro_http_resposta_400(cliente_http, requests_mock):
    ...
    with pytest.raises(OpenAIBadRequestError) as info_excecao: # ALTERADO
...
def test_erro_http_resposta_401(cliente_http, requests_mock):
    ...
    with pytest.raises(OpenAIAuthenticationError) as info_excecao: # ALTERADO
...
def test_erro_http_resposta_404(cliente_http, requests_mock):
    ...
    with pytest.raises(OpenAINotFoundError) as info_excecao: # ALTERADO
        cliente_http.obter(ponto_final_teste)
...
def test_erro_http_resposta_429_com_retries(cliente_http, requests_mock):
    ...
    with pytest.raises(OpenAIRetryError) as info_excecao: # ALTERADO
        cliente_http.obter(ponto_final_teste)
    ...
    assert isinstance(info_excecao.value.original_exception, requests_mock.requests.exceptions.HTTPError) # ALTERADO
    assert info_excecao.value.original_exception.response.status_code == 429 # ALTERADO
...
def test_erro_http_resposta_500_com_retries(cliente_http, requests_mock):
    ...
    assert "Máximo de retries (1) excedido" in str(info_excecao.value)
    assert isinstance(info_excecao.value.original_exception, requests_mock.requests.exceptions.HTTPError) # ALTERADO
    assert info_excecao.value.original_exception.response.status_code == 500 # ALTERADO
    assert requests_mock.call_count == 2 # Deve ter feito 2 chamadas (original + 1 retry)
...
def test_excecao_tempo_limite_com_retries(cliente_http, requests_mock):
    ...
    assert "Máximo de retries (1) excedido" in str(info_excecao.value)
    assert isinstance(info_excecao.value.original_exception, Timeout) # ALTERADO
...
def test_excecao_conexao_com_retries(cliente_http, requests_mock):
    ...
    assert "Máximo de retries (1) excedido" in str(info_excecao.value)
    assert isinstance(info_excecao.value.original_exception, requests_mock.requests.exceptions.ConnectionError) # ALTERADO
    assert requests_mock.call_count == 2 # Deve ter feito 2 chamadas
